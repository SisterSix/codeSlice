...
//    private static int DELTA = 0x9e3779c8;
    ...
    public static byte[] encrypt(byte[] data, byte[] key, int len) {
        ...
        return toByteArray(
                encrypt(toIntArray(data, true, len), toIntArray(fixKey(key), false)), false);
    }
    ...
    public static byte[] encrypt(byte[] data, String key, int len) {
        try {
            return encrypt(data, key.getBytes("UTF-8"), len);
        } catch (UnsupportedEncodingException e) {
            return null;
        }
    }
    ...
    public static byte[] decrypt(byte[] data, byte[] key, int len) {
        ...
        return toByteArray(
                decrypt(toIntArray(data, false, len), toIntArray(fixKey(key), false)), true);
    }
    ...
    public static byte[] decrypt(byte[] data, String key, int len) {
        try {
            return decrypt(data, key.getBytes("UTF-8"), len);
        } catch (UnsupportedEncodingException e) {
            return null;
        }
    }
    ...
    private static int[] toIntArray(byte[] data, boolean includeLength, int len) {
        int n = (((len & 3) == 0)
                ? (len >>> 2)
                : ((len >>> 2) + 1));
        int[] result;
